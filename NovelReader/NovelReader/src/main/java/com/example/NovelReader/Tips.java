...
public class Tips {
    ...
    private static View createToastView(String message) {
        ...
        drawable.getPaint().setStyle(Paint.Style.FILL);// 置画笔样式,如果不设置,默认是全部填充
        drawable.getPaint().setAntiAlias(true);// 抗锯齿方法
        drawable.getPaint().setFlags(Paint.ANTI_ALIAS_FLAG);//消除锯齿
        ...
        textView.setLineSpacing(dp2px(4), 1f);//行间距
